...
parser.add_argument('-name', default='ft_v3_mix', help='model save name')
...
# additional configs:
parser.add_argument('--pretrained', default='ckpt/pretrain/pt_v3_mix_model.pth', type=str,
                    help='path to moco pretrained checkpoint')
parser.add_argument('--frozen', default=False, type=bool, help='only fine-tuning fc')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        model = vits.__dict__[args.arch](patch_size=args.patch_size, embed_dim=args.embed_dim, depth=args.depth,
                                         img_size=args.input_size, num_heads=args.num_heads)
        linear_keyword = 'head'
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            state_dict = get_model_from_moco(checkpoint, linear_keyword)
    ...
    assert not args.frozen or len(parameters) == 2  # weight, bias
    ...
    if args.resume:
        if not args.evaluate:
            acc_record = torch.load(os.path.join(args.output_dir, args.name + '_acc.log'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.save(acc_record, os.path.join(args.output_dir, args.name + '_acc.log'))
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, filename=os.path.join(args.output_dir, args.name + '_model.pth'))
            if args.pretrained and args.frozen and epoch == args.start_epoch:
...
def get_model_from_moco(checkpoint, linear_keyword):
    # rename moco pre-trained keys
